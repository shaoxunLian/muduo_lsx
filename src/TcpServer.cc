...
//防止TcpServer构造函数中传入的loop是空的，空的就没有事件循环了
...
TcpServer::~TcpServer()
{
    ...
    for(auto &item : connections_)
    {
        ...
        //释放托管对象的所有权（如果有），调用后，该智能指针不再管理任何对象。注意，这里的释放并不是真正地去释放，
        //只是引用计数减 1，而释放是引用计数为 0 时自行进行的。就是让本智能指针指向空，释放其指向，也就是引用计数减1.
        item.second.reset();
...
void TcpServer::newConnection(int sockfd, const InetAddress &peerAddr)
{
    ...
    //getsockname()函数用于获取与某个套接字关联的本地协议地址(本端地址)
    //getpeername()函数用于获取与某个套接字关联的外地协议地址(对端的地址)
    if(getsockname(sockfd, (sockaddr*)&local, &addrlen) < 0)//可获得服务端本地的本地协议地址

...
EpollPoller::EpollPoller(EventLoop* loop)
    : Poller(loop),
      epollfd_(epoll_create1(EPOLL_CLOEXEC)),//epoll_create1参数可以是 0 或者 EPOLL_CLOEXEC
...
{
    /**
     * 先获取channel的状态，根据channel的状态调用update()，其实就是调用epoll_ctl注册或修改或删除这个channel
     * 对应的fd的在epoll中的监听事件。
     * 
     */
...
{
    /**
     *  从epoll里面删掉这个channel，然后更改这个channel的状态，从Poller的channel_map删除这个channel
     * 
     */
    int fd = channel->fd();
    channels_.erase(fd);
...
{
    /**
     *  poll其实就是调用epoll或poll，然后向activeChannels填入有事件发生的channel！
     * 
     */
    LOG_DEBUG("func=%s => fd total count: %lu \n", __FUNCTION__, channels.size());
    //epoll_wait:观察list链表里有没有数据。有数据就返回，没有数据就sleep，等到timeout时间到后即使链表没数据也返回。

...
#include <memory>
/*
//头文件中只用到EventLoop的指针，而未调用其成员函数或成员变量，可使用其他类的前置申明，这样就不需要提供相应的头文件给客户；
Channel.cpp源文件中有用到EventLoop类的成员方法，所以需要在源文件中包含EventLoop.h头文件，但是源文件最终会被编译成so库，且不需要提供相应头文件出去
参考https://www.zhihu.com/question/447193147
 */
class EventLoop;
...
private:
    ...
    std::weak_ptr<void> tie_; //这个tie_ 绑定了TcpConnection
    bool tied_;

...
1. 为什么要用enable_shared_from_this?
    * 需要在类对象的内获得一个指向当前部中对象的shared_ptr 对象
...
//TcpConnection都是在subLoop里面管理的               //因为TcpConnection需要提供给用户，所以也需要采用该继承方式
class TcpConnection : public muduo::noncopyable, public std::enable_shared_from_this<TcpConnection>
{
    /**TcpConenction 存在于 subLoop里面 ，Acceptor存在于mainLoop ，可相似看待两者 
     * TcpConnection包含了Buffer成员，Buffer打包了readv()和write(),即负责发送数据和接收数据的功能
     *  TcpConnection主要用于打包成功连接的TCP连接通信链路。
...
private:
    ...
    //这里和Acceptor类似Acceptor=》mainLoop ， TcpConenction=》subLoop
    ...
    WriteCompleteCallback writeCompleteCallback_; //消息发送完后的回调//chatserver没设置
    HighWaterMarkCallback highWaterMarkCallback_;//发送和接收缓冲区超过水位线的话容易导致溢出，这是很危险的。//本项目没设置
    CloseCallback closeCallback_;
    size_t highWaterMark_; //水位线
    Buffer inputBuffer_; //接收的缓冲区大小，
    Buffer outputBuffer_; //发送的缓冲区大小，
};